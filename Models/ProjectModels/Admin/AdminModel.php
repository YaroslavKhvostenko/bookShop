...
class AdminModel extends AbstractAdminModel
{
    ...
    public function getAdmins(string $actionName): ?array
    {
        ...
        $this->setDbMsgModel();
        $dbResult = $this->db->select($this->getDbFields($actionName))
            ->from(['admins'])->condition($this->getConditionData($actionName), $andOr)->query()->fetchAll();
        if (!$dbResult) {
            $this->msgModel->setMsg('empty', 'no_admins', 'empty');
    ...
    public function changeAccess(array $data, string $fieldName): void
    {
        ...
        if ($fieldName === 'is_head' && $count > 1) {
            $this->msgModel->setMsg('failure', 'too_much_admins', 'too_much_admins');
        ...
        if ($dbResult) {
            $this->msgModel->setMsg('success', $this->getRefererAction(), 'success');
            if ($fieldName === 'is_head') {
                $conditionData = $this->formatArrayData(['id'], $this->sessionModel->getUserData());
                $dbResult = $this->db->update(['admins'], ['is_head' => 0])->condition($conditionData)->exec();
                if ($dbResult) {
                    $this->sessionModel->setUserData(['is_head' => '0']);
                } else {
                    $this->logger->log(
                        'default',
                        'Problems to update db data of previous HeadAdmin '.
                        $this->sessionModel->getUserData()['name'] . ':(' .
                        $this->sessionModel->getUserData()['login'] . ')!'
                    );
                }
            }
        } else {
            throw new \Exception(
                'Something went wrong during updating Admin DB data,
                 using : ' . $this->getRequestAction() . 'Action->changeAccess methods!'
            );
        }
    }
     ...
     * @return array
     */
    private function formatArrayData(array $fieldNames, array $data): array
    {
        $result = [];
        ...
        if (empty($result)) {
            throw new InvalidArgumentException(
                'Cannot find field in provided array data!'
            );
