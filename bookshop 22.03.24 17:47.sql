...
--
-- Хост: localhost:3306
-- Время создания: Мар 22 2024 г., 17:44
...
INSERT INTO `admins` (`id`, `login`, `pass`, `name`, `birthdate`, `email`, `phone`, `address`, `image`, `is_active`, `is_admin`, `is_approved`, `is_head`) VALUES
(1, 'HeadAdmin', '$2y$10$RjwCcanhmbO3Z1tzhasUHujLfqPSqddz2ndvulgXy4cIr1jOqGimi', 'HeadAdmin', '13.01.1993', 'head_admin@gmail.com', '+380111111111', 'head admin house', NULL, 1, 1, 1, 1);
...
ALTER TABLE `admins`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `admins_login` (`login`),
  ADD UNIQUE KEY `admins_email` (`email`),
  ADD UNIQUE KEY `admins_phone` (`phone`);
...
ALTER TABLE `tasks`
  ...
  ADD KEY `tasks_admin_id_tasks_task_status` (`admin_id`,`task_status`);
...
--
ALTER TABLE `tasks`
  ADD CONSTRAINT `tasks_ibfk_1` FOREIGN KEY (`admin_id`) REFERENCES `admins` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
COMMIT;
